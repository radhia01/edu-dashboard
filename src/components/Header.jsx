...
import { AiOutlineContainer } from "react-icons/ai";
  const CardItem=({title,number,icon,color})=>{
    ...
    <div className={`w-[300px] rounded-sm py-4 px-3  h-[150px]   ${color}  grid grid-cols-3   items-center `}>
        <div className="text-4xl   flex justify-center  ">{icon} </div>
        <div className="    col-span-2  flex  flex-col ">
            <h1 className='uppercase fon-bold '>{title}</h1><h1 className='text-3xl font-bold '>{number}</h1>
        </div>
    </div>)
